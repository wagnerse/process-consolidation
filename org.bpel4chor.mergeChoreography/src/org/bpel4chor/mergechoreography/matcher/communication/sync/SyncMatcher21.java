 ...
 * 
 */
public class SyncMatcher21 implements SyncMatcher {
	...
	public MergePattern match(MessageLink mlSend, MessageLink mlReply, ChoreographyPackage pkg) {
		...
		this.pattern = new SyncPattern21(this.envSend, this.envReply, pkg);
		
		((SyncPattern21) this.pattern).setmLinkSend(mlSend);
		
		// Set the conditions
		Condition cond1 = new Condition(this.envSend.getR() instanceof Pick);
		
		this.results.add(cond1.evaluate());
		this.log.info("SyncMatcher 2.1 R() instanceof Pick : " + cond1.evaluate());
