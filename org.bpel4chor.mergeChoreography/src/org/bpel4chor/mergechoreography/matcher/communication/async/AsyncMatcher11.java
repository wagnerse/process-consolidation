 ...
 * 
 */
public class AsyncMatcher11 implements LinkMatcher {
	...
	public CommunicationPattern match(MessageLink link, ChoreographyPackage choreographyPackage) {
		...
		if (link.getSenders().size() == 0) {
			...
			// receive
			// The same for the receive activity but the following invoke
			// should correspond to the second receive
			...
			// be another invoke and the activity following the invoke should
			// be another receive
			if (BPEL4ChorModelHelper.isNonCommunicatingActivity(a1) && BPEL4ChorModelHelper.isNonCommunicatingActivity(b1) && (a2 instanceof Receive) && (b2 instanceof Invoke)) {
				// Now check if a2 invokes b2
				Receive receive2 = (Receive) a2;
				Invoke invoke2 = (Invoke) b2;
				if (BPEL4ChorModelHelper.findByInvokeActivity(invoke2, choreographyPackage).getReceiveActivity().equals(receive2.getName())) {
					...
					this.logger.log(Level.INFO, "AsyncPattern11 found for link => " + link.getName() + " with sending Process => " + environment.getSender().getName() + ",  and receiving Process => " + environment.getReceiver().getName() + ".");
					this.logger.log(Level.INFO, "       (A1)        (B1)");
					this.logger.log(Level.INFO, "        |           |");
					this.logger.log(Level.INFO, "    (Invoke)  -> (Receive)");
					this.logger.log(Level.INFO, "        |           |");
					this.logger.log(Level.INFO, "   (Receive2) <- (Invoke2)");
					this.logger.log(Level.INFO, "With A1 : " + a1.getName());
					this.logger.log(Level.INFO, "With Invoke : " + inv.getName());
					this.logger.log(Level.INFO, "With Receve2 : " + a2.getName());
					this.logger.log(Level.INFO, "With B1 : " + b1.getName());
					this.logger.log(Level.INFO, "With Receive : " + rec.getName());
					this.logger.log(Level.INFO, "With Invoke2 : " + b2.getName());
					...
					return new AsyncPattern11(link, choreographyPackage, environment);
				}
			}
		}
		return null;
