 ...
 * 
 */
public class AsyncMatcher14 implements AsyncMatcher {
	...
	/** The {@link AsyncPattern14} of this {@link AsyncMatcher} */
	...
	public MergePattern match(MessageLink link, ChoreographyPackage pkg) {
		...
		this.pattern = new AsyncPattern14(this.env, pkg);
		
		// Set the conditions
		Condition cond1 = new Condition(this.env.getSuccS().size() == 0);
		Condition cond2 = new Condition(this.env.getR() instanceof Receive);
		Condition cond3 = new Condition(this.env.getSuccR().size() == 0);
		
		this.results.add(cond1.evaluate());
		this.log.info("AsyncMatcher 1.4 SuccS().size() == 0 : " + cond1.evaluate());
		this.results.add(cond2.evaluate());
		this.log.info("AsyncMatcher 1.4 R() instanceof Receive : " + cond2.evaluate());
		this.results.add(cond3.evaluate());
		this.log.info("AsyncMatcher 1.4 SuccR().size() == 0 : " + cond3.evaluate());
