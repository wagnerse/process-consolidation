...
import org.bpel4chor.mergechoreography.pattern.communication.async.AsyncPattern10;
 ...
 * 
 */
public class AsyncMatcher10 implements LinkMatcher {
	...
	public CommunicationPattern match(MessageLink link, ChoreographyPackage choreographyPackage) {
		...
		if (link.getSenders().size() == 0) {
			...
			// involved invoke, the same for the receive activity
			...
			if (BPEL4ChorModelHelper.isNonCommunicatingActivity(a1) && BPEL4ChorModelHelper.isNonCommunicatingActivity(a2) && BPEL4ChorModelHelper.isNonCommunicatingActivity(b1) && BPEL4ChorModelHelper.isNonCommunicatingActivity(b2)) {
				...
				return new AsyncPattern10(link, choreographyPackage, environment);
			}
		}
		return null;
	}
