...
import org.bpel4chor.mergechoreography.pattern.communication.async.AsyncPattern3;
 ...
 * 
 */
public class AsyncMatcher3 implements LinkMatcher {
	...
	public CommunicationPattern match(MessageLink link, ChoreographyPackage choreographyPackage) {
		...
		if (link.getSenders().size() == 0) {
			...
			// The same for the receive activity
			if ((environment.getSendBeforeEnvironment().size() == 1) && (environment.getSendAfterEnvironment().size() == 0) && (environment.getRecBeforeEnvironment().size() == 1) && (environment.getRecAfterEnvironment().size() == 0)) {
			...
			// non-communicating
			if (BPEL4ChorModelHelper.isNonCommunicatingActivity(a1) && BPEL4ChorModelHelper.isNonCommunicatingActivity(b1)) {
				// We found our Pattern !!!
				this.logger.log(Level.INFO, "AsyncPattern3 found for link => " + link.getName() + " with sending Process => " + environment.getSender().getName() + ",  and receiving Process => " + environment.getReceiver().getName() + ".");
				this.logger.log(Level.INFO, "       (A1)        (B1)");
				this.logger.log(Level.INFO, "        |           |");
				this.logger.log(Level.INFO, "    (Invoke)  -> (Receive)");
				this.logger.log(Level.INFO, "        |           |");
				this.logger.log(Level.INFO, "        O           O");
				...
				return new AsyncPattern3(link, choreographyPackage, environment);
			}
