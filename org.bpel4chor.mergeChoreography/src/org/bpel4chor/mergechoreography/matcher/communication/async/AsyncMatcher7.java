...
import org.bpel4chor.mergechoreography.pattern.communication.async.AsyncPattern7;
 ...
 * 
 */
public class AsyncMatcher7 implements LinkMatcher {
	...
	public CommunicationPattern match(MessageLink link, ChoreographyPackage choreographyPackage) {
		...
		if (link.getSenders().size() == 0) {
			...
			// be another receive
			if (BPEL4ChorModelHelper.isNonCommunicatingActivity(a1) && BPEL4ChorModelHelper.isNonCommunicatingActivity(b1) && (b2 instanceof Invoke)) {
				// We found our Pattern !!!
				this.logger.log(Level.INFO, "AsyncPattern7 found for link => " + link.getName() + " with sending Process => " + environment.getSender().getName() + ",  and receiving Process => " + environment.getReceiver().getName() + ".");
				this.logger.log(Level.INFO, "       (A1)        (B1)");
				this.logger.log(Level.INFO, "        |           |");
				this.logger.log(Level.INFO, "    (Invoke)  -> (Receive)");
				this.logger.log(Level.INFO, "        |           |");
				this.logger.log(Level.INFO, "        O        (Invoke2)");
				...
				return new AsyncPattern7(link, choreographyPackage, environment);
			}
