...
import org.bpel4chor.mergechoreography.pattern.communication.async.AsyncPattern2;
 ...
 * 
 */
public class AsyncMatcher2 implements LinkMatcher {
	...
	public CommunicationPattern match(MessageLink link, ChoreographyPackage choreographyPackage) {
		...
		if (link.getSenders().size() == 0) {
			...
			// The same for the receive activity, but with a following
			// activity
			...
			// non-communicating
			if (BPEL4ChorModelHelper.isNonCommunicatingActivity(a1) && BPEL4ChorModelHelper.isNonCommunicatingActivity(b1) && BPEL4ChorModelHelper.isNonCommunicatingActivity(b2)) {
				// We found our Pattern !!!
				this.logger.log(Level.INFO, "AsyncPattern2 found for link => " + link.getName() + " with sending Process => " + environment.getSender().getName() + ",  and receiving Process => " + environment.getReceiver().getName() + ".");
				this.logger.log(Level.INFO, "       (A1)        (B1)");
				this.logger.log(Level.INFO, "        |           |");
				this.logger.log(Level.INFO, "    (Invoke)  -> (Receive)");
				this.logger.log(Level.INFO, "        |           |");
				this.logger.log(Level.INFO, "        O          (B2)");
				...
				return new AsyncPattern2(link, choreographyPackage, environment);
			}
