...
import org.bpel4chor.mergechoreography.pattern.communication.async.AsyncPattern1;
 ...
 * 
 */
public class AsyncMatcher1 implements LinkMatcher {
	...
	public CommunicationPattern match(MessageLink link, ChoreographyPackage choreographyPackage) {
		...
		if (link.getSenders().size() == 0) {
			...
			// preceding activity and one succeeding activity for the
			// involved invoke
			// The same for the receive activity, but without any following
			// activities
			if ((environment.getSendBeforeEnvironment().size() == 1) && (environment.getSendAfterEnvironment().size() == 1) && (environment.getRecBeforeEnvironment().size() == 1) && (environment.getRecAfterEnvironment().size() == 0)) {
			...
			// Now we know that invoke has preceding and succeeding activity,
			// and receive has one preceding
			// Check if all are non-communicating
			if (BPEL4ChorModelHelper.isNonCommunicatingActivity(a1) && BPEL4ChorModelHelper.isNonCommunicatingActivity(a2) && BPEL4ChorModelHelper.isNonCommunicatingActivity(b1)) {
				this.logger.log(Level.INFO, "AsyncPattern1 found for link => " + link.getName() + " with sending Process => " + environment.getSender().getName() + ",  and receiving Process => " + environment.getReceiver().getName() + ".");
				this.logger.log(Level.INFO, "       (A1)        (B1)");
				this.logger.log(Level.INFO, "        |           |");
				this.logger.log(Level.INFO, "    (Invoke)  -> (Receive)");
				this.logger.log(Level.INFO, "        |           |");
				this.logger.log(Level.INFO, "       (A2)         O");
