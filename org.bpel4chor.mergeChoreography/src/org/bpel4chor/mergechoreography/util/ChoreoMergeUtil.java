...
public class ChoreoMergeUtil {
	 ...
	 * 
	 * @param name
	 *            The name of the {@link ParticipantType}
	 * @param topology
	 *            The {@link Topology} of the Choreography
	 * @return
	 */
	 ...
	 * 
	 * @param topology
	 *            {@link Topology} of the choreo
	 * @param partName
	 *            {@link Participant} to find the type for
	 ...
	 * @return {@link List} of {@link Activity}s if there are some, else an
	 *         empty {@link List}
	 */
	 ...
	 * 
	 * @param act1
	 *            The {@link Activity} with searched sources
	 * @param act2
	 *            The {@link Activity} with corresponding targets
	 ...
	 * 
	 * @param activity
	 *            The {@link Activity} containing the source
	 * @param linkName
	 *            The name of the link of the source
	 ...
	 * 
	 * @param activity
	 *            The {@link Activity} containing the target
	 * @param linkName
	 *            The name of the link of the target
	 ...
	 * 
	 * @param activity
	 *            {@link Activity} to remove source from
	 * @param linkName
	 *            The name of the link of the source
	 */
	 ...
	 * 
	 * @param activity
	 *            {@link Activity} to remove target from
	 * @param linkName
	 *            The name of the link of the target
	 */
	 ...
	 * 
	 * @param activity
	 *            The {@link Activity} to add the target to
	 * @param target
	 *            The {@link Target} to be added
	 */
	 ...
	 * 
	 * @param activity
	 *            The {@link Activity} to add the source to
	 * @param target
	 *            The {@link Source} to be added
	 */
	 ...
	 * 
	 * @param act
	 *            {@link Activity} to determine jc from
	...
	public static void removeActivityFromContainer(Activity act) {
		...
		if (container instanceof Flow) {
			...
			if (flow.getActivities().size() == 0) {
				...
				if (((flow.getSources() == null) || (flow.getSources().getChildren().size() == 0))
						&& ((flow.getTargets() == null) || (flow.getTargets().getChildren().size() == 0))) {
			...
			if (seq.getActivities().size() == 0) {
				// Check if there are no <sources> and <targets> remained
				if (((seq.getSources() == null) || (seq.getSources().getChildren().size() == 0))
						&& ((seq.getTargets() == null) || (seq.getTargets().getChildren().size() == 0))) {
			...
			if (((scp.getSources() == null) || (scp.getSources().getChildren().size() == 0))
					&& ((scp.getTargets() == null) || (scp.getTargets().getChildren().size() == 0))) {
	 ...
	 * 
	 * @param actOld
	 *            {@link Activity} to be replaced
	 * @param actNew
	 *            The new {@link Activity}
	 */
	...
	/**
	 * Check for all succeeding {@link Activity}s i.e check if the given
	 * {@link Activity} is in a loop else check if there is an {@link Activity}
	 * after the root of the given {@link Activity}
	 * 
	 * @param act
	 *            The {@link Activity} to check
	 * @return All Succeeding {@link Activity}s, null else
	 */

	public static List<Activity> getAllSucceedingActivities(Activity act) {

		List<Activity> after = getSucceedingActivities(act);
		List<Activity> allAfter = new ArrayList<Activity>();
		Boolean inLoop = false;
		Activity succAct = null;

		if (after == null) {
			ChoreoMergeUtil.log.info("************** ALL SUCC *************");
			if (ChoreoMergeUtil.isElementInLoop(act)) {
				//TODO Make it better
				allAfter.add(act);
				inLoop = true;
			}
			...
			while (result != null && !inLoop) {
				Activity rootAct = (Activity) result[0];
				result = ChoreoMergeUtil.getSequenceAndInsideActivity(rootAct);
				if (result != null) {
			...
			if (allAfter != null) {
				if (inLoop) {
					ChoreoMergeUtil.log.info("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
					ChoreoMergeUtil.log
							.info("The activity : " + act + " is in a loop, hence do have succeeding activity");
					ChoreoMergeUtil.log.info("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
				} else {
					ChoreoMergeUtil.log.info("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
					ChoreoMergeUtil.log.info("Found following succeeding Activity in <sequence> for act : " + act);
					ChoreoMergeUtil.log.info(" => " + succAct);
					ChoreoMergeUtil.log.info("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
				}

			}
			return (allAfter != null ? allAfter : null);
		} else
	 ...
	 * 
	 * @param wsuID
	 *            The wsu:id of the searched {@link Activity}
	 * 
	 ...
	 * 
	 * @param elem
	 *            The {@link BPELExtensibleElement} to search for the
	 ...
	 * 
	 * @param varName
	 *            The name of the {@link Variable} to be resolved
	 * @param act
	 *            The {@link Activity} using the {@link Variable}
	 ...
	 * 
	 * @param act
	 *            {@link Activity} to get the highest {@link Scope} of
	...
	public static Scope getHighestScopeOfActivity(Activity act) {
		...
		while (!(container instanceof Process)) {
			...
			if (((Flow) container).getName().equals("MergedFlow")) {
				...
				ChoreoMergeUtil.log
						.info("Found <scope> " + ((Scope) containerSub).getName() + " for activity " + act.getName());
	 ...
	 * {@link Process}. It's the next {@link Scope} containing act
	 * {@link Activity}.
	 * 
	 * @param act
	 *            {@link Activity} to get the Parent {@link Scope} of
	...
	public static Scope getParentScopeOfActivity(Activity act) {
		...
		while (!(container instanceof Process)) {
			if (container instanceof Scope) {
				// We found the containing <scope>
				ChoreoMergeUtil.log
						.info("Found <scope> " + ((Scope) container).getName() + " for activity " + act.getName());
		...
		for (Link link : flow.getLinks().getChildren()) {
			if (link.getName().equals(newLink.getName())) {
				throw new RuntimeException(
						"There's already a link " + newLink.getName() + " in flow " + flow.getName());
			}
		}
	 ...
	 * 
	 * @param throw
	 *            The {@link Throw} in emulated EH-Scope
	 * @param actsource
	 *            The {@link Activity} in EH-instantiating-Scope
	 */
	 ...
	 *            The {@link Link}
	 * @param act
	 *            The {@link Activity} for the new {@link Target}
	 ...
	 *            The {@link Link}
	 * @param act
	 *            The {@link Activity} for the new {@link Source}
	 ...
	 * 
	 * @param act
	 *            {@link Activity} with {@link Target}s
	 ...
	 * 
	 * @param s
	 *            {@link Invoke} with {@link From} part {@link Variable}
	 * @param vR
	 *            {@link Variable} to {@link Copy} {@link To}
	 ...
	 * 
	 * @param act
	 *            {@link PartnerActivity} with {@link From} part
	 *            {@link Variable}
	 * @param vR
	 *            {@link Variable} to {@link Copy} {@link To}
	 * @return new {@link Assign}
	...
	public static Assign createAssignFromSendAct(PartnerActivity act, Variable vR) {
		...
		newCopy.setTo(newTo);
		newAssign.setElement(act.getElement());
		return newAssign;
	}
	...
	public static Scope createScopeFromInvoke(Invoke s, Variable vR) {
		...
		// Move FaultHandlers
		if ((s.getFaultHandler() != null)
				&& ((s.getFaultHandler().getCatch().size() > 0) || (s.getFaultHandler().getCatchAll() != null))) {
	 ...
	 * 
	 * @param act
	 *            {@link Activity} to analyse attribute of
	 * @return true or false
	 */
	 ...
	 * 
	 * @param empty
	 *            The {@link Empty}-{@link Activity} to be optimized
	 */
		...
		if (ChoreoMergeUtil.getSuppressJoinFailureAttribute(empty)) {
			...
			if ((preActs.size() > 0) || (succActs.size() > 0)) {
				...
				for (Activity succAct : succActs) {
					...
					for (Activity preAct : preActs) {
						...
						if (pre2e != null) {
							...
							if ((tc2succAct != null) || (tcPreAct != null)) {
								...
								cmbdTC.setBody((tc2succAct != null ? "(" + tc2succAct + ")" : "")
										+ (tcPreAct != null ? " and (" + tcPreAct + ")" : ""));
								nsPre2SuccAct.setTransitionCondition(cmbdTC);
							}

							// Adapt JCs
							if (jcEmpty == null) {
								jcSuccAct = XPathUtil.replaceVariableName(jcSuccAct, e2sAct.getName(),
										nPre2succAct.getName());
							} else {
								jcEmpty = XPathUtil.replaceVariableName(jcEmpty, pre2e.getName(),
										nPre2succAct.getName());
							}
					...
					if (e2sAct == null) {
						// Combine JCs
						jcNew = (jcEmpty != null ? "(" + jcEmpty + ")" : "")
								+ (jcSuccAct != null ? " and (" + jcSuccAct + ")" : "");
					} else {
						if (jcEmpty != null) {
	 ...
	 * 
	 * @param act
	 *            {@link Activity} from which we want to propagate the attribute
	 ...
	 * 
	 * @param act
	 *            {@link Activity} to propagate initiate-attribute to
	 * @param corSets
	 *            {@link List} of names of {@link CorrelationSet}s
	 */
	 ...
	 * 
	 * @param act
	 *            {@link Activity} to create {@link Empty} from
	 ...
	 * 
	 * @param act
	 *            {@link Activity} to create {@link Scope} from
	 * @return new {@link Scope}
	 */
	 ...
	 * 
	 * @param pick
	 *            {@link Pick} to be transformed
	 * @param om
	 *            {@link OnMessage} branch to be ommitted
	 * @param vGuard
	 *            Guard {@link Variable} to be used in {@link Assign}s
	 * @return new {@link Scope}
	 */
	public static Scope createScopeFromPickWithoutOM(Pick pick, List<OnMessage> om, Variable vGuard) {
		...
		for (OnAlarm onAlarm : pick.getAlarm()) {
			...
			QName qName = new QName(ChoreoMergeUtil.pkg.getMergedProcess().getTargetNamespace(),
					"wait" + (counter++) + "Fault");
			Throw newThrow = ChoreoMergeUtil.createThrowForUserDefinedFault(qName);
	 ...
	 * 
	 * @param onAlarm
	 *            {@link OnAlarm} to create {@link Wait} from
	 ...
	 * 
	 * @param om
	 *            {@link OnMessage} branch to create new {@link Receive} from
	 * @param createInstance
	 *            Attribute from surrounding {@link Pick}
	 ...
	 * 
	 * @param inv
	 *            {@link Invoke} to create {@link If} from
	 * @param gVar
	 *            Guard {@link Variable}
	 * @param vR
	 *            {@link Receive}ing {@link Variable}
	 * @return new {@link If}
	 */
	...
	/**
	 * Create new {@link Throw} for user defined Fault within simulated
	 * EventHandler
	 * 
	 * @return new {@link Throw}
	 */
	 ...
	 * 
	 * @param fh
	 *            {@link Catch}-Fault Handler to add
	 * @param elem
	 *            {@link BPELExtensibleElement} to add the {@link Catch} to
	 */
	...
	public static boolean isActivityInFCTEHandler(Activity act) {
		EObject container = act.eContainer();
		while (!(container instanceof Process)) {
			if ((container instanceof FaultHandler) || (container instanceof CompensationHandler)
					|| (container instanceof TerminationHandler) || (container instanceof EventHandler)) {
				return true;
			}
			// Climb up
			container = container.eContainer();
	 ...
	 * 
	 * @param elem
	 *            {@link BPELExtensibleElement} to check
	 * @return true or false
	 */
	...
	public static boolean isElementInFCTEHandler(BPELExtensibleElement elem) {
		List<Class<? extends BPELExtensibleElement>> typesToCheck = Arrays.asList(FaultHandler.class,
				CompensationHandler.class, TerminationHandler.class, EventHandler.class);
		return ChoreoMergeUtil.isElementContainedIn(elem, typesToCheck);
	}

	/**
	 * Check whether given {@link BPELExtensibleElement} is in an @{link
	 * EventHandler}
	 * 
	 * @param elem
	 *            {@link BPELExtensibleElement} to check
	 ...
	 *            {@link BPELExtensibleElement} to check
	 * @param list
	 *            {@link List} of {@link BPELExtensibleElement}
	 * @return true or false
	 */
	public static boolean isElementContainedIn(BPELExtensibleElement elem,
			List<Class<? extends BPELExtensibleElement>> list) {
		EObject container = elem.eContainer();
	 ...
	 * 
	 * @param act
	 *            {@link Activity} to get FCTE-Handler of
	...
	public static BPELExtensibleElement getFCTEHandlerOfActivity(Activity act) {
		...
		while (!(container instanceof Process)) {
			if ((container instanceof Catch) || (container instanceof CatchAll)
					|| (container instanceof CompensationHandler) || (container instanceof TerminationHandler)
					|| (container instanceof OnEvent) || (container instanceof OnAlarm)) {
	...
	/**
	 * Get the {@link EventHandler} which contains the given
	 * {@link BPELExtensibleElement}
	 * 
	 * @param act
	 *            {@link BPELExtensibleElement} to get {@link EventHandler} of
	 ...
	 * 
	 * @param v
	 *            {@link Variable} to uplift the declaration of
	 * @param process
	 *            {@link Process} for new declaration
	 */
	public static void upliftVariableToProcessScope(Variable v, Process process) {
		if (v.eContainer().eContainer() instanceof Process) {
			...
			ChoreoMergeUtil.log.info(
					"Variable already in process " + process.getName() + " no need to uplift variable " + v.getName());
		} else {
			...
			for (Variable variable : process.getVariables().getChildren()) {
				if (variable.getName().equals(v.getName())) {
					throw new RuntimeException("Problem : There's already another variable declared in process "
	 ...
	 * 
	 * @param act
	 *            {@link Activity} to combine joinCondition in
	 * @param link
	 *            {@link Link} to be combined in new joinCondition
	 */
	public static void combineJCWithLink(Activity act, Link link) {
		...
		if (act.getTargets() != null) {
			if (act.getTargets().getJoinCondition() != null) {
				...
				if (!jcOld.equals("")) {
					ChoreoMergeUtil.log.info("jcOld " + jcOld);
					jcNew += " and (" + jcOld + ")";
				}
			}
	...
	public static Flow getMergedFlow(Activity act) {
		...
		// EObject container = act.eContainer();
		// while (!(container instanceof Process)) {
		// // TODO: Nested Flows and Scopes would cause a casting error here.
		// since first act of process
